...
declare -A ARCH_SERVICES=(
    ["bpc4msa"]="business-logic"
    ["synchronous"]="soa-service"
    ["monolithic"]="monolithic-service"
)
...
WARMUP_SECONDS=15  # Wait for services to stabilize
...
monitor_resources() {
    ...
    timeout "${duration}s" docker stats "$service_name" \
        --no-stream=false \
...
calculate_resource_averages() {
    ...
    awk -F',' '{
        ...
        # Parse memory (extract first number, assume MiB)
        split($2, mem_parts, " ")
        gsub(/MiB/, "", mem_parts[1])
        mem_sum += mem_parts[1]
...
execute_load_test() {
    ...
    # Try locust command, fall back to python -m locust (fully silent)
...
execute_trial() {
    ...
    printf "  ${BLUE}⚡${NC} Running load test (${TEST_DURATION}s)... "
    local locust_csv
